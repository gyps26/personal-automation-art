...
import gypsonProfile from "@/assets/portfolio/gypson-profile.jpg";
...
export function About() { return <section id="about" className="border-t border-border py-16"><div className="mx-auto max-w-7xl px-4 sm:px-6"><div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr]"><div><span className="text-sm font-semibold text-primary">About me</span><h2 className="mt-3 text-3xl font-semibold sm:text-4xl">Gypson Feguro</h2><p className="mt-2 font-medium text-foreground">Automation Specialist &amp; GoHighLevel Funnel Expert</p><p className="mt-5 leading-7 text-muted-foreground">3+ years in no-code/low-code platforms specializing in workflow automation, API integrations, and process optimization. Building efficient business automations that reduce manual tasks by 20-25%. Expert in designing high-converting GoHighLevel funnels that streamline lead capture, nurture sequences, and appointment booking to maximize client conversions.</p><div className="mt-7 space-y-2 text-sm"><a className="block text-primary hover:underline" href="tel:+639669718411">+639669718411</a><a className="block text-primary hover:underline" href="mailto:gypsonfeguro1@gmail.com">gypsonfeguro1@gmail.com</a><p className="text-muted-foreground">Tandag City, Surigao del Sur, Philippines</p></div></div><div><div className="mb-7 aspect-square max-w-sm overflow-hidden rounded-md border border-border bg-secondary/40"><img src={gypsonProfile} alt="Gypson Feguro" className="h-full w-full object-cover"/></div><div className="mb-3 flex items-center justify-between"><h3 className="text-xl font-semibold">Technical Skills</h3><span className="platform-pill platform-n8n">LIVE</span></div><div className="flex flex-wrap gap-2">{skills.map(skill=><span className="feature-chip px-3 py-2 text-xs" key={skill}>{skill}</span>)}</div></div></div>
...
export function Footer() { return <footer className="border-t border-border"><div className="mx-auto max-w-7xl px-4 py-8 text-sm text-muted-foreground sm:px-6"><span>© {new Date().getFullYear()} Gypson Feguro. All rights reserved.</span></div></footer>; }
